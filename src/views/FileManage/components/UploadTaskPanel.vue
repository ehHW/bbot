<template>
    <section
        ...
        :class="{ 'upload-task-panel--dragging': isDragging }"
    ...
    >
        <div v-if="isDragging" class="upload-task-panel__drop-mask">
            <div class="upload-task-panel__drop-card">
                <div class="upload-task-panel__drop-title">释放文件或文件夹</div>
                <div class="upload-task-panel__drop-desc">导入后继续选择上传目录</div>
            </div>
        </div>
        ...
        <div class="upload-toolbar-row">
            ...
            <span class="upload-toolbar-row__status">{{ uploadQueueStatusText }}</span>
        </div>

        <a-progress v-if="fileStore.uploadTasks.length" :percent="fileStore.overallUploadProgress" style="margin-bottom: 12px" />

        <div v-if="!fileStore.uploadTasks.length" class="upload-empty-state">
            <div class="upload-empty-state__title">拖拽文件或文件夹到这里</div>
            <div class="upload-empty-state__desc">也可以点击下面的按钮选择文件，随后再指定上传目录。</div>
            <a-space wrap>
        ...
        >
            <div class="target-modal-toolbar">
                ...
                <a-button type="primary" ghost size="small" @click="toggleCreateFolderInline">新建文件夹</a-button>
            </div>

            <div v-if="createFolderInlineOpen" class="target-modal-toolbar__inline">
                <a-input
                ...
                />
                <a-button type="primary" size="small" :loading="createFolderSaving" @click="handleCreateFolder">创建</a-button>
                <a-button size="small" :disabled="createFolderSaving" @click="handleCancelCreateFolder">取消</a-button>
            </div>
...
const folderEntries = computed(() => fileStore.entries.filter((item) => item.is_dir && !item.is_recycle_bin))
...
const queueFilesForUpload = async (files: File[]) => {
    ...
    await fileStore.loadEntries(fileStore.currentParent?.is_recycle_bin ? null : fileStore.currentParentId)
...
const cloneFileWithRelativePath = (file: File, relativePath: string) => {
    const cloned = new File([file], file.name, { type: file.type, lastModified: file.lastModified }) as File & { webkitRelativePath?: string }
...
const readFileEntry = (entry: FileSystemEntryWithFile, relativePath: string) =>
    new Promise<File>((resolve, reject) => {
        entry.file?.(
            (file) => resolve(cloneFileWithRelativePath(file, relativePath || file.name)),
            (error) => reject(error),
        )
    })

const readDirectoryEntries = (reader: NonNullable<ReturnType<NonNullable<FileSystemEntryWithFile['createReader']>>>) =>
    new Promise<FileSystemEntry[]>((resolve, reject) => {
        reader.readEntries(resolve, reject)
    })

const collectEntryFiles = async (entry: FileSystemEntryWithFile, currentPath = ''): Promise<File[]> => {
    ...
    while (true) {
        ...
        for (const child of entries) {
            const childFiles = await collectEntryFiles(child as FileSystemEntryWithFile, nextPath)
...
const confirmUploadToCurrentFolder = async () => {
    ...
    try {
        await fileStore.addUploadFiles(waitingFiles.value, fileStore.currentParentId)
        waitingFiles.value = []
