<template>
    <a-card>
        <div class="settings-page__toolbar">
            <span class="save-status" :class="`save-status--${saveStatus}`">{{ saveStatusText }}</span>
        </div>
        <a-form layout="vertical" :model="formState">
            <a-divider orientation="left">基础设置</a-divider>
            <a-form-item label="系统标题">
                <a-input v-model:value="formState.systemTitle" placeholder="请输入系统标题" @blur="handleSystemTitleBlur" @press-enter="handleSystemTitleEnter" />
            </a-form-item>
            <a-form-item label="主题模式">
                <a-segmented v-model:value="formState.themeMode" :options="themeOptions" @change="handleThemeModeChange" />
            </a-form-item>

            <a-divider orientation="left">聊天偏好</a-divider>
            <a-form-item label="接收聊天通知">
                <a-switch v-model:checked="formState.chatReceiveNotification" checked-children="开启" un-checked-children="关闭" @change="handleChatReceiveNotificationChange" />
            </a-form-item>
...
onMounted(() => {
    syncFromStore()
    settingsStore.loadChatPreferences().then(syncFromStore).catch(() => undefined)
})
