<template>
    <a-modal :open="open" title="选择资源中心文件" :footer="null" :width="CHAT_MODAL_WIDTH_LG" @cancel="handleCancel">
        <a-alert
            type="info"
            show-icon
        ...
        <a-space style="margin-bottom: 12px; width: 100%; justify-content: space-between" wrap>
            <a-breadcrumb>
                <a-breadcrumb-item v-for="item in breadcrumbs" :key="String(item.id ?? 'root')">
        ...
        >
            <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'actions'">
                    <a-space>
                        <a v-if="canSelect(record)" @click="selectItem(record)">选择</a>
                        <a v-else-if="record.is_dir" @click="enterFolder(record)">进入</a>
...
const tableData = computed<Array<FileEntryItem | SearchFileEntryItem>>(() => (isSearchMode.value ? searchResults.value : entries.value))
...
const canSelect = (item: FileEntryItem | SearchFileEntryItem) => !item.is_dir && Boolean(item.asset_reference_id)
