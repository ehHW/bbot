...
- 聊天附件入口、资源中心入口、上传目标目录入口已共用统一 `AssetPickerSelection` 协议；上传目录不再维护独立返回结构
- `uploadFileWithCategory()` 已统一返回标准化资产结果 `selection`，页面层不再直接拆 small-file / instant / chunked 的差异字段
- B2 本轮范围已收口：聊天现有资产插入、上传后插入、撤回恢复、附件复制、聊天记录条目复制/转发都已并到统一 asset prepare / clipboard 层
- C1 首轮已落地：资源中心文件列表和搜索接口的主要分支编排已收口到 `hyself/application/queries/resource_center.py`
...
- [x] 把聊天附件入口、资源中心入口、上传目标目录入口继续收敛到统一 asset picker 协议
- [x] 统一 upload 完成后的资产结果结构，减少页面层感知 small-file / chunked 差异
- [x] 为聊天侧补一层 asset prepare / attachment payload 统一组装入口
- [x] 为 `hyself/views.py` 选定第一组 query/service 下沉接口
...
- 上传目标目录选择已切到同一套 `selectionMode: 'folder'` + `AssetPickerSelection` 输出，不再维护独立目录返回结构

下一步：

- 把“保存到资源中心”目标目录选择等特殊入口继续对齐到同一套 scene / selection 协议
...
- 已新增统一 `AssetPreviewModel` 与 `assetPreview` 工具，先收口 displayName、mediaType、mimeType、fileSize、url、thumbnailUrl、processingStatus 等最小预览字段
- `AssetPickerSelection`、`UploadFileResult`、`UploadTaskItem` 已挂接统一 `preview`，聊天 asset prepare 也开始直接消费同一份 preview 数据
- `FileNameCell` 已开始消费统一 preview；资源中心列表、资产选择器列表、上传队列都已复用这份最小预览模型

下一步：

- 把聊天消息气泡、更多 asset/file card 和上传完成提示继续切到同一份 preview 数据
- 再决定是否抽独立 asset card / file card 组件，而不是先为组件而组件
...
- 明确文件图标、缩略图、文件大小、来源、资源状态的展示规则

验收标准：

- 图片和文件至少能在聊天、资源中心列表、上传队列三处共用同一套字段模型
- 不再新增第三套只服务单页的资产展示结构
...
- `uploadFileWithCategory()` 已统一返回标准化 `selection`，调用方优先消费资产结果而不是上传模式细节
- 聊天附件发送前的资产结果已经通过 uploader 内部统一标准化，不再从 `result.file?.asset` 一路手动拼字段

下一步：

- 继续把这份结果协议复用到 B2 的聊天 asset prepare 层，减少附件 token 与消息 payload 的重复组装
...
- 已新增面向聊天的 asset prepare 层，用统一入口承接“选已有资产”和“上传后得到资产”的 composer token / 消息 payload 组装
- 本地图片/文件选择与粘贴已切到“先上传为 chat asset，再插入 composer token”流程，不再保留发送时临时上传的旧链路
- 资源中心现有资产插入聊天输入框、上传完成后插入 token、撤回消息恢复附件 token，已经共用同一层准备逻辑
- 聊天记录条目复制/转发、资产消息复制等剩余入口也已统一复用 prepare/clipboard helper，不再各自手拼 token / HTML / 文本

下一步：

- 视需要补一层更显式的 prepare 状态反馈，避免上传耗时较长时用户感知不足
- richer media 再进入时，优先扩展 prepare 输出，不再回头改上传分支和 token 组装逻辑
...
- 图片、普通文件、聊天记录条目、撤回恢复和附件复制至少共享同一层消息前置准备逻辑
...
- `FileEntriesAPIView` 与 `SearchFileEntriesAPIView` 的主要系统/用户分支编排已下沉到 `hyself/application/queries/resource_center.py`
- `hyself/views.py` 现在主要保留权限校验、参数解析、HTTP 响应映射与异常到状态码的翻译

下一步：

- 继续观察回收站列表、上传分片查询等剩余读接口是否还需要第二轮下沉
- 把首轮 query facade 的命名与目录约束，继续衔接到 C2 的 assets 域命名约定
...
1. 推进 A2 第二轮：把聊天消息气泡、更多 asset/file card、上传完成提示继续切到同一份 preview 模型
2. 收尾 A1 特殊场景：把“保存到资源中心”目标目录选择等剩余入口并到统一 picker 交互层
3. 推进 C2：把首轮 query / command 落点整理成 assets 域目录和命名约定
4. 视体验情况补一轮聊天附件准备中的状态反馈和错误提示文案
